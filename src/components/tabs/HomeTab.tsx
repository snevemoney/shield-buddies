...
export const HomeTab: React.FC<{ onNavigate: (tab: string) => void }> = ({ onNavigate }) => {
  ...
  // Calculate estimated days of food supply based on quantity and consumption rate.
  // Assumes ~3 food units per person per day (meals). Only count non-expired items.
  ...
  const totalFoodUnits = foodSupplies.reduce((sum, s) => {
    ...
    // Exclude expired items (d <= 0), include items with no expiry date (d === null)
    if (d !== null && d <= 0) return sum;
    // Normalize to roughly equivalent units: packs/cans/boxes count as 1 meal each
    if (s.unit === 'kg') return sum + s.quantity * 4; // ~4 meals per kg
    if (s.unit === 'cans' || s.unit === 'packs' || s.unit === 'boxes') return sum + s.quantity;
    return sum + s.quantity;
  }, 0);
  const foodDays = memberCount > 0
      ...
      <div className="space-y-4 pb-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button onClick={() => onNavigate('supplies')} className="bg-card border border-border rounded-xl p-4 text-left hover:bg-accent/50 transition-colors">
            ...
            <div className="text-2xl font-bold font-mono-data text-foreground">{foodDays}</div>
